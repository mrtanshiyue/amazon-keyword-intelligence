...
> 2026-09-03 Inventory / Anomaly 阻断级卡死修复：`growth-consistency-actions.js` 的 Inventory 后置补丁此前每次都用 `innerHTML` 重建 Risk badge，而 `import-workspace-states.js` 又会监听 `#content` subtree 并重建顶部 Import Readiness 卡片；卡片重建会再次触发 Growth consistency 的顶层刷新，两个 observer 因非幂等子节点写入形成跨模块反馈环。现在 Risk badge 只在 class/text 真正变化时原位更新，不再无条件增删节点；Anomaly Center 的库存告警行增加 managed marker + 内容 fingerprint，相同模型不再重复 remove/append。回归测试覆盖这条 cross-observer 收敛边界。CI 为 **428 passed / 0 failed**；`npm run build` 验证 **49 个 JS + 9 个 CSS，59 个发布文件**，source/dist byte identity 与 committed-dist parity gate 全部通过。CI 为 **426 passed / 0 failed**；`npm run build` 验证 **49 个 JS + 9 个 CSS，59 个发布文件**，source/dist byte identity 与 committed-dist parity gate 继续生效。
