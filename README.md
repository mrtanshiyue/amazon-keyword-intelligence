...
Completed product workflows include Dashboard/Analytics, Ad Manager, Suggestions, supported local Rules, Action Center/Change Log, Keyword Research, Keyword Tracker, Keyword/Negative libraries, Conflict Guard, Protected Keywords, Unified Transaction analytics, browser-local Ads/Unified imports, Local Data Operations, Store management, responsive/mobile hardening, keyboard accessibility, URL page history, suite workspace homes, suite-aligned Growth navigation taxonomy, first-class Listing preparation, CSV-first Search Query Funnel, Product 360, Action Outcomes, Rank & Index Tracker, Listing Optimizer 2.0, Inventory Risk, deterministic Anomaly Center, imported competitor snapshot trends/seasonality evidence, storefront/new-listing snapshot comparisons, imported competitor Ads Insights, manual competitor image/selling-point evidence capture and exact-tag comparison, review marketplace/language normalization, explicit review VOC labels, CSV-first contextual page controls, controlled mobile data tables, import-workspace readiness states/schema guidance, evidence trend/drill-down panels, evidence-first suite home intelligence, source-aware dynamic Chinese/English/bilingual runtime coverage including generated filter/accessibility labels, canonical Tracker/Listing workflow aliases, browser-local Workspace Organizer, imported market structure/opportunity screening, local purchase-order planning CSV export, and a read-only KeywordOS Agent over validated Store 01 Dataset Registry evidence with scoped specialist analysis modes and deterministic natural-language metric-change diagnostics.
...
- [x] Unified Dataset Registry covering Ads, Unified, SQP/ABA, cost, inventory, rank, competitor, competitor Ads, competitor creative evidence, review and Listing datasets.
...
- [x] Read-only KeywordOS Agent over `validated` Store 01 Dataset Registry records. The Assistance drawer uses deterministic local intent routing for data inventory, Ads search-term evidence, inventory, rank/index, reviews, competitor snapshots, finance transactions and SQP evidence; every answer exposes source provenance, coverage and original source-row numbers where applicable, excludes other Stores/non-validated records, fails closed without evidence, and never calls external AI/Amazon APIs or creates actions.
- [x] Specialist analysis modes for knowledge/help, advertising, keyword, Listing, profitability and inventory questions. One mode selector scopes the existing read-only Agent rather than creating parallel assistants: Advertising stays on imported Ads evidence; Keyword uses SQP/rank evidence; Listing reads the validated local draft without generating copy; Profitability requires validated Product Master + Ads + Unified finance + Cost datasets and exact identifier joins before calculating loaded-period contribution/margin/break-even ACoS; Inventory reads imported snapshots; Help stays on supported capabilities/data provenance. Missing prerequisites fail closed.
- [x] Natural-language “why did this metric change?” diagnostics with deterministic evidence before any narrative. The Agent recognizes English/Chinese why-change questions and compares Store 01 `validated` evidence only: Ads Spend/Sales/Orders/Clicks/Impressions/ACoS/ROAS/CPC/CVR use equal observed-date windows; inventory and rank use the two latest dated snapshots; SQP demand/purchases and finance Total/Fees/Refund use equal observed-date windows. It shows exact formulas, source provenance, largest grouped arithmetic deltas and original source-row numbers before explanatory text, and explicitly labels grouped movement as arithmetic contribution rather than causal inference. Missing history fails closed; no external AI, Amazon-side event, algorithm cause or remote action is invented.
- [ ] AI-assisted Listing drafting with optional product-image understanding and explicit human approval. This remains pending until a real model/vision provider and an explicit human-review/approval boundary are connected; deterministic rules must not be labelled as AI drafting or image understanding.
- [x] Competitor image/selling-point comparison with evidence capture. Competitor Snapshots now accepts browser-local manual evidence containing ASIN, capture date, image slot, image reference/URL/file ID, explicit visual tags, explicit selling-point labels and a source note in the Store 01 `competitor-creative` Dataset Registry kind. Comparison uses the latest captured evidence independently per ASIN/image slot and exact case-insensitive label sets to show shared / ASIN-A-only / ASIN-B-only image-slot coverage, visual tags and selling-point labels. Saving the same ASIN/date/slot replaces that captured row as a correction; no image inspection, OCR, synonym matching, inferred claim, traffic/conversion/ranking inference or compliance conclusion is performed.
...
- [x] Products, Keywords, Marketing, Operations and Analytics suite homes lead with evidence-first KPI row counts, Dataset Registry health, explicit dated coverage and deterministic next-task shortcuts before the existing tool cards. KPI values are persisted/imported row counts or loaded-source counts only; missing/explicitly invalid essential sources drive EMPTY/PARTIAL/READY and task priority without AI inference. The Listing top-suite action intentionally opens the canonical Listing Optimizer directly instead of maintaining a separate link-directory home.
- [x] Direct CSV-first workspaces replace the global Store/date/import header controls with page-specific exact-match selectors over the rendered imported evidence: Marketplace for Product Master, ASIN for Search Funnel/Rank/Competitor/Review, Keyword Set for ASIN Comparison and SKU for Inventory. The existing page-local import/template actions remain authoritative, Store 01 imported-data scope is labelled explicitly, and leaving those pages restores the original global controls.
- [x] Legacy `tracker` / Keyword Tracker and `listing-workspace` / Listing Workspace entry points are compatibility aliases to the canonical Rank & Index Tracker (`rank-intelligence`) and Listing Optimizer (`listing-optimizer`). Legacy sidebar, suite-card and command-palette entries are hidden without deleting local data; old `#page=` hashes are replaced with the canonical route, and the Listing top-suite action opens Listing Optimizer directly.
- [ ] Consolidate Analytics surfaces only after an explicit product-scope decision: Portfolio Overview, Cross-store Intelligence and Advertising Analytics currently represent different scopes, so KeywordOS does not redirect or merge them merely to reduce page count.
- [x] Chinese / English / bilingual mode now reapplies automatically to newly rendered UI through the existing i18n runtime: dynamic messages, generated statuses/actions, table headers and recent product companion surfaces share one translation dictionary and MutationObserver path. Runtime `placeholder` / `title` / `aria-label` changes are source-aware, so controls such as the sidebar toggle do not fall back to English after interaction, and newly inserted root elements translate their own accessible attributes. Generated CSV exact-filter labels/options and mobile table region labels also stay language-stable across Chinese / English / bilingual switching. Imported evidence cells such as Keyword, Title, Body, ASIN, SKU, Search Query and explicit review/VOC labels remain verbatim; plain table-cell translation is restricted to generated status/validation/decision columns, while `data-no-i18n` remains an explicit opt-out.
- [x] Search Funnel, Rank, Inventory, Outcomes and Anomalies expose evidence-first trend panels plus drill-down drawers. Search Funnel aggregates imported query demand/purchases by explicit date; Rank shows median positive observed organic/sponsored ranks; Inventory sums available/inbound units; Outcomes charts imported Ads spend/sales only as non-causal context; Anomalies charts dated Ads/Unified/Inventory evidence-row coverage rather than fabricated anomaly counts. Missing dates/metrics remain unavailable, current CSV-first ASIN/SKU filters are reapplied by exact value only, and drawers retain persisted source/coverage/import/validation/checksum provenance plus raw evidence rows.
- [x] Mobile data tables use one controlled horizontal-scroll treatment across existing `table-wrap`, `table-scroll` and otherwise-unwrapped `#content` tables. On narrow screens the primary identity column is pinned, a bilingual scroll hint is shown, touch momentum/overscroll containment is enabled, and focusable table regions support Arrow Left/Right scrolling; desktop table layout and business data remain unchanged.
- [x] Import-driven workspaces expose explicit EMPTY / LOADING / ERROR / PARTIAL / READY states plus required/optional schema guidance. All eight KeywordOS Growth CSV parser kinds, storefront snapshots and competitor Ads observations provide downloadable parser-aligned templates; the Ads/Unified Import Center uses the same readiness surface but intentionally requires real source exports instead of fabricating Amazon report templates.
- [x] Browser-local Workspace Organizer provides saved views over real `#page=` routes, preserves the existing CSV-first exact-match selector when one is active, groups views into projects, supports deduplicated tags and favorites, and records up to 12 real recent page/filter visits as shortcuts. Saved filters are reapplied only when the exact value still exists in current imported rows; organizer metadata is included in the existing Local Workspace backup/restore allowlist.
...
- **Keywords** — suite home for existing keyword intelligence workspaces
