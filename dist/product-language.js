(function(root){
  ...
  function productLabel(page,label){const value=String(label??'');if(clean(page)===LEGACY_KEYWORD_ROUTE&&value.trim()===LEGACY_KEYWORD_LABEL)return KEYWORD_LAB_LABEL;return value;}
  ...
  function start(){if(!root?.document||!registry)return;const boot=()=>{applyProductLanguage(root.document);root.addEventListener('keywordos:page-rendered',()=>applyProductLanguage(root.document));root.document.querySelectorAll('[data-lang]').forEach(button=>button.addEventListener('click',()=>setTimeout(()=>applyProductLanguage(root.document),0)));};root.document.readyState==='loading'?root.document.addEventListener('DOMContentLoaded',boot,{once:true}):boot();}
  const api={LEGACY_KEYWORD_ROUTE,LEGACY_KEYWORD_LABEL,KEYWORD_LAB_LABEL,KEYWORD_RESEARCH_LABEL,SUITE_ZH,SUITE_DETAIL_ZH,SIDEBAR_ZH,PAGE_ZH,productLabel,replaceLegacyKeywordLabel,pageLanguage,suiteLanguage,sidebarLabel,toolWorkspaceLabel,apply:applyProductLanguage,applyProductLanguage,start};
