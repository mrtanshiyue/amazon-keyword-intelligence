...
const PUBLIC_API={STORAGE_KEY,MAX_PRESETS,FILTER_KEYS,COLUMN_CATALOG,DEFAULT_SORT,clean,modeId,catalog,allowedKeys,normalizeColumns,moveColumn,normalizeSort,normalizeFilterSnapshot,normalizePreset,normalizeState,upsertPreset,deletePreset,rowValue,sortRows,csvQuote,rowsToCsv,normalizeKeywordSelection};
...
function normalizeKeywordSelection(values){const seen=new Set(),out=[];for(const raw of Array.isArray(values)?values:[]){const keyword=clean(raw),key=keyword.toLowerCase();if(!keyword||seen.has(key))continue;seen.add(key);out.push(keyword);}return out;}
