...
const RAW_PAGES=[
  ...
  ['cerebro','keywords','keywords','⌕','Cerebro','Cerebro','Research and qualify keyword opportunities using Amazon advertising search-term data.','core',70],
...
const PAGES=Object.freeze(RAW_PAGES.map(([id,suite,sidebarGroup,icon,navLabel,title,subtitle,source,order])=>Object.freeze({
  id,suite,sidebarGroup,icon,navLabel,title,subtitle,source,order,
  eyebrow:sidebarGroup==='settings'?'SETTINGS':String(suite||sidebarGroup).toUpperCase(),
  i18nKey:`page.${id}`,
  command:true,
  nav:true,
  legacy:false,
...
const VIRTUAL_PAGES=Object.freeze(SUITE_ORDER.filter(suite=>SUITES[suite].home).map(suite=>Object.freeze({
  id:SUITES[suite].home,suite,sidebarGroup:'',icon:'',navLabel:SUITES[suite].title,title:SUITES[suite].title,subtitle:SUITES[suite].subtitle,source:'virtual',order:0,eyebrow:suite.toUpperCase(),i18nKey:`page.${SUITES[suite].home}`,command:true,nav:false,legacy:false,virtual:true
})));
...
function decorateButton(button,record){button.dataset.pageRegistryKey=record.id;button.dataset.i18nKey=record.i18nKey;button.dataset.suite=record.suite||'';const label=button.querySelector('.nav-label');if(label&&label.textContent.trim()!==record.navLabel)label.textContent=record.navLabel;if(record.navLabel)button.title=record.navLabel;}
function organizeGrowthNavigation(){const nav=root?.document?.getElementById('sidebar-nav');if(!nav)return false;const sections=sectionMap(nav),buttons=[...nav.querySelectorAll('[data-page]')];for(const button of buttons){const raw=button.dataset.page||'';if(registry.isLegacy(raw))continue;const record=registry.page(raw);if(!record||!record.nav||!record.sidebarGroup)continue;decorateButton(button,record);const section=sections.get(record.sidebarGroup);if(section&&button.parentElement!==section)section.appendChild(button);}for(const group of NAV_SIDEBAR_ORDER){const section=sections.get(group);if(!section)continue;for(const record of registry.sidebarGroups().find(item=>item.group===group)?.pages||[]){const button=[...section.querySelectorAll(':scope > [data-page]')].find(item=>registry.canonicalPage(item.dataset.page)===record.id);if(button)section.appendChild(button);}}for(const section of [...nav.querySelectorAll(':scope > .nav-section')]){if(section.dataset.registrySection)continue;if(!section.querySelector('[data-page]'))section.remove();}for(const group of NAV_SIDEBAR_ORDER){const section=sections.get(group);if(!section)continue;const title=section.querySelector(':scope > .nav-section-title');if(title){title.textContent=group.toUpperCase();title.dataset.i18nKey=`sidebar.${group}`;}}const desired=NAV_SIDEBAR_ORDER.map(group=>sections.get(group)).filter(Boolean),current=[...nav.children].filter(section=>section.dataset?.registrySection);if(desired.some((section,index)=>current[index]!==section))for(const section of desired)nav.appendChild(section);root?.KeywordOSI18N?.apply?.(nav);return true;}
