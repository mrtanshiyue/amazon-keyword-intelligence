...
'use strict';
const PAGE_ALIASES=Object.freeze({tracker:'rank-intelligence','listing-workspace':'listing-optimizer'});
const LEGACY_PAGES=Object.freeze(Object.keys(PAGE_ALIASES));
function cleanPage(value){return String(value||'').trim();}
function canonicalPage(page){const value=cleanPage(page);return PAGE_ALIASES[value]||value;}
...
function listingSuiteTarget(label){return cleanPage(label).toLowerCase()==='listing'?'listing-optimizer':'';}
function shouldHidePage(page){return LEGACY_PAGES.includes(cleanPage(page));}
...
let scheduled=false;
function canonicalButton(page){return $$('#sidebar-nav [data-page]').find(button=>button.dataset.page===page&&!button.hidden)||null;}
function writeCanonicalHash(page){const hash=`#page=${encodeURIComponent(page)}`;if(root.location.hash!==hash)root.history.replaceState({keywordOSPage:page},'',hash);}
