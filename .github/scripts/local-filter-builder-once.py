...
'''
Path('local-filter-builder.js').write_text(module)
...
'''
Path('tests/local-filter-builder.test.mjs').write_text(tests)

replace_once('index.html','  <script src="csv-page-controls.js"></script>\n  <script src="mobile-data-tables.js"></script>','  <script src="csv-page-controls.js"></script>\n  <script src="local-filter-builder.js"></script>\n  <script src="mobile-data-tables.js"></script>','index local filter wiring')
replace_once('package.json','node --check csv-page-controls.js && node --check mobile-data-tables.js','node --check csv-page-controls.js && node --check local-filter-builder.js && node --check mobile-data-tables.js','package check local filter')
replace_once('package.json','review-voc.js csv-page-controls.js mobile-data-tables.js','review-voc.js csv-page-controls.js local-filter-builder.js mobile-data-tables.js','package build local filter')
replace_once('local-operations-actions.js',"    'keywordos_v9_keyword_lab_view',\n", "    'keywordos_v9_keyword_lab_view',\n    'keywordos_v9_local_filter_builder',\n",'backup local filter key')
replace_once('README.md','- [ ] 增加可编辑的 Simple / Advanced 本地筛选与 Top-N 条件构造器；保存条件，不保存伪装成实时的结果。','- [x] 增加可编辑的 Simple / Advanced 本地筛选与 Top-N 条件构造器；保存条件，不保存伪装成实时的结果。\n  - 2026-09-03：新增轻量 `local-filter-builder.js`，复用现有 `csv-page-controls.js` 的 7 个 CSV-first 页面边界，不改 parser、Dataset Registry 或任何证据行。Builder 自动选择当前页面 tbody 行数最多的主数据表，Simple 只允许 1 条条件；Advanced 最多 5 条并支持 ALL / ANY。条件字段完全来自当前表头，支持 contains / not contains / equals / not equals / > / ≥ / < / ≤ / between / missing / present；货币、千分位和百分比只按当前单元格显示值透明解析，不生成缺失指标。Top-N 在条件和既有 Marketplace / ASIN / SKU 上下文筛选之后执行，可选 Top / Bottom、1–1000 和当前可数值化字段；缺字段或没有可数值值时 fail-closed，不展示伪结果。状态只在 `keywordos_v9_local_filter_builder` 保存 `{page, mode, join, conditions, topN}` 与命名 preset，preset 按“页面 + 名称”大小写无关更新；`normalizeState()` 会丢弃 results / resultRows / resultCount / evaluatedAt 等结果型字段，重新加载或数据刷新后都从当前渲染导入表重新计算。它只用独立 DOM hidden 标记做可逆视图过滤，不重排数据，也不会覆盖现有 CSV context 的隐藏状态。CI 为 **__TEST_COUNT__ passed / 0 failed**；`npm run build` 验证 **__JS_COUNT__ 个 JS + 9 个 CSS，__PUBLISH_COUNT__ 个发布文件**，source/dist byte identity 与 committed-dist parity gate 全部通过。','README P3 local filter item')
